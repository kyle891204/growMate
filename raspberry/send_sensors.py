...
# ── 설정 ────────────────────────────────────────────────────────
SERVER_URL   = "http://172.20.10.3:8000/api/sensors"
    ...
    try:
        while True:
            try:
                ...
                # 1) 표정 업데이트 (백엔드 없이도 독립 동작)
                if FACE_ENABLED:
                    update_face(data)
 
                # 2) 서버 전송
                r = requests.post(SERVER_URL, json=data, timeout=5)
                print("보냄", data, "->", r.status_code, r.text[:80])
 
            except requests.exceptions.ConnectionError:
                print("[경고] 서버 연결 실패 — 표정은 계속 표시됩니다.")
                if FACE_ENABLED:
                    update_face(data if 'data' in dir() else
